...
def servico_criar(request: HttpRequest) -> HttpResponse:
    empresas = empresas_disponiveis_para_usuario(request.user).order_by('nome_fantasia', 'razao_social')
    campos_obrigatorios = {
        'titulo': 'Informe o título do serviço.',
        'categoria': 'Selecione uma categoria.',
        'descricao_curta': 'Informe uma descrição curta.',
        'descricao_completa': 'Informe a descrição completa.',
        'cidade': 'Informe a cidade de atendimento.',
    }
    erros = {}
    ...
    if request.method == 'POST':
        ...
        prestador_tipo = request.POST.get('prestador_tipo', 'pf')
        if prestador_tipo not in {'pf', 'pj'}:
            erros['prestador_tipo'] = 'Selecione um tipo de prestador válido.'
        if prestador_tipo == 'pj' and not request.POST.get('empresa'):
            erros['empresa'] = 'Selecione uma empresa vinculada.'
        elif prestador_tipo == 'pj' and not any(
            str(empresa.pk) == request.POST.get('empresa') for empresa in empresas
        ):
            erros['empresa'] = 'Selecione uma empresa vinculada válida.'
        if not request.POST.getlist('tipo_atendimento'):
            erros['tipo_atendimento'] = 'Selecione ao menos um tipo de atendimento.'

        acao = request.POST.get('acao', 'rascunho')
        if acao not in {'rascunho', 'publicar'}:
            erros['acao'] = 'Ação inválida.'

        if not erros:
            mensagem_acao = 'publicação' if acao == 'publicar' else 'salvamento como rascunho'
            messages.info(
                request,
                f'Protótipo validado para {mensagem_acao}. Nenhum dado foi gravado nesta etapa.',
            )
            return redirect('painel:servicos_lista')

        messages.error(request, 'Revise os campos obrigatórios destacados.')

    return render(
        request,
        'painel/servicos/novo.html',
        {
            'titulo': 'Novo cadastro de serviço',
            'empresas': empresas,
            'erros': erros,
            'tipos_atendimento': request.POST.getlist('tipo_atendimento'),
        },
    )
...
@login_required
def servico_links(request: HttpRequest, pk: int) -> HttpResponse:
    servico = _servico_autorizado(request, pk)
    if not usuario_pode_editar_servico(request.user, servico):
        raise PermissionDenied
    if request.method == 'POST':
        ...
        if excluir:
            ...
            return redirect('painel:servico_links', pk=servico.pk)
    ...
    return render(request, 'painel/links/gerenciar.html', {'objeto': servico, 'tipo_objeto': 'serviço', 'form': form, 'links': servico.links.filter(excluido_em__isnull=True), 'voltar_url': reverse('painel:servico_detalhe', args=[servico.pk])})


@login_required
def empresa_links(request: HttpRequest, pk: int) -> HttpResponse:
    empresa = _empresa_autorizada(request, pk)
    if not usuario_pode_editar_empresa(request.user, empresa):
        raise PermissionDenied
    if request.method == 'POST':
        ...
        if excluir:
            ...
            messages.success(request, 'Link removido com sucesso.')
            return redirect('painel:empresa_links', pk=empresa.pk)
    ...
    return render(request, 'painel/links/gerenciar.html', {'objeto': empresa, 'tipo_objeto': 'empresa', 'form': form, 'links': empresa.links.filter(excluido_em__isnull=True), 'voltar_url': reverse('painel:empresa_detalhe', args=[empresa.pk])})
...
def servico_qrcode(request: HttpRequest, uuid) -> HttpResponse:
    ...
    return render(request, 'painel/qrcode.html', {'objeto': servico, 'tipo_objeto': 'serviço', 'url_curta': url, 'imagem_url': f'{request.path}?formato=png', 'voltar_url': reverse('painel:servico_detalhe', args=[servico.pk])})
...
def empresa_qrcode(request: HttpRequest, uuid) -> HttpResponse:
    ...
    return render(request, 'painel/qrcode.html', {'objeto': empresa, 'tipo_objeto': 'empresa', 'url_curta': url, 'imagem_url': f'{request.path}?formato=png', 'voltar_url': reverse('painel:empresa_detalhe', args=[empresa.pk])})
