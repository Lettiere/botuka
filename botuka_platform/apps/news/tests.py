...
class NewsTests(TestCase):
    def setUp(self):
        self.user=get_user_model().objects.create_user('reporter',password='x');p=Perfil.objects.create(nome='NEWS_REPORTER');self.user.perfil=p;self.user.save();perm=Permissao.all_objects.get(codigo='news.criar');PerfilPermissao.objects.create(perfil=p,permissao=perm);perm_edit=Permissao.all_objects.get(codigo='news.editar_propria');PerfilPermissao.objects.create(perfil=p,permissao=perm_edit);self.cat=CategoriaNoticia.objects.create(nome='Cidade')
