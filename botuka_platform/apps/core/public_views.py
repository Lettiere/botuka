...
def eventos_lista(request):
    ...
    cadastrados = Evento.objects.filter(status=Evento.Status.PUBLICADO, publico=True, inicio__date__gte=hoje).select_related('empresa_promotora', 'proprietario')
    if termo:
        ...
        cadastrados = cadastrados.filter(Q(titulo__icontains=termo) | Q(resumo__icontains=termo) | Q(descricao__icontains=termo) | Q(local__icontains=termo) | Q(organizador__icontains=termo))
    if bairro:
        oficiais = oficiais.filter(bairro__iexact=bairro)
        cadastrados = cadastrados.filter(endereco__icontains=bairro)
    if categoria == "cultura":
        ...
        cadastrados = cadastrados.filter(categoria__iexact=categoria)
    if tipo == "municipal": esportivos = esportivos.none()
    if tipo == "municipal": cadastrados = cadastrados.none()
    if tipo == "esportivo":
        oficiais = oficiais.none()
        cadastrados = cadastrados.none()
    ...
    if inicio:
        ...
        cadastrados = cadastrados.filter(inicio__date__range=(inicio, fim))
    ...
    itens += [EventoPublicoDTO(e.uuid, e.titulo, e.resumo, e.categoria or 'Evento', e.inicio, e.fim, e.local, e.organizador or (e.empresa_promotora.nome_fantasia if e.empresa_promotora_id else e.proprietario.get_full_name()), False, None, _imagem(e.imagem_principal), reverse('events:detalhe', args=[e.slug]), 'BOTUKA') for e in cadastrados[:100]]
    ordem = request.GET.get("ordem")
    def data_ordenacao(item):
        inicio_item = item.inicio
        if isinstance(inicio_item, datetime):
            inicio_item = timezone.localtime(inicio_item).date() if timezone.is_aware(inicio_item) else inicio_item.date()
        return inicio_item if isinstance(inicio_item, date) else date.max
    itens.sort(key=(lambda x: x.titulo.casefold()) if ordem == "az" else data_ordenacao)
